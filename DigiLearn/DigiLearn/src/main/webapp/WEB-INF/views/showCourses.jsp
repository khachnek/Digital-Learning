...
<body>
	<div class="jumbotron text-center">
		<h1>All courses related to ${type}</h1>
	</div>


	<c:forEach items="${course}" var="courses">
		<ol>
			<option value="${courses.key}">${courses.key}</option>
			<option value="${courses.value}"><a
					href="<c:url value = "${courses.value}"/>">${courses.value}</a>
			</option>
		</ol>
	</c:forEach>
